...
router.get('/', requireAuth, async (req, res) => {
  ...
  const [items, total] = await prisma.$transaction([
    prisma.measurement.findMany({ where, orderBy: { date: 'desc' }, skip, take: pageSize, include: { institution: true, sector: true, files: true } }),
...
router.post('/', requireAuth, async (req, res) => {
  ...
  const m = await prisma.measurement.create({ data: { ...data, status, userId: user.id, date: new Date(data.date) } })
  res.json({ id: m.id })
})
...
router.put('/:id', requireAuth, async (req, res) => {
  const data = req.body
  const status = computeStatus(data)
  await prisma.measurement.update({ where: { id: req.params.id }, data: { ...data, status, date: new Date(data.date) } })
  res.json({ ok: true })
})
...
router.post('/:id/files', requireAuth, upload.array('files', 5), async (req, res) => {
  ...
  const created = await prisma.$transaction(files.map(f => prisma.file.create({ data: { measurementId: id, name: f.originalname, path: '/uploads/' + path.basename(f.path), mime: f.mimetype, size: f.size, category: category ?? undefined } })))
...
router.get('/bi', requireAuth, async (req, res) => {
  ...
  const series = items.map(i => ({ date: i.date.toISOString().slice(0,10), temperature: i.temperature, humidity: i.humidity }))
...
router.get('/report', requireAuth, async (req, res) => {
  ...
  const items = await prisma.measurement.findMany({ where, orderBy: { date: 'desc' }, take, include: { institution: true, sector: true } })
  if (String(format) === 'excel') {
    const wb = new ExcelJS.Workbook()
    const ws = wb.addWorksheet('Medições')
    ws.addRow(['Data','Instituição','Setor','Temp (°C)','Umidade (%)','CO2 Int (ppm)','CO2 Ext (ppm)','Status'])
    for (const i of items) ws.addRow([i.date.toISOString(), i.institution?.name ?? i.institutionId, i.sector?.name ?? i.sectorId, i.temperature, i.humidity, i.co2Internal, i.co2External, i.status])
  ...
  items.forEach(i => {
    ...
    doc.text(`${s} | ${nmI} > ${nmS} | Temp: ${i.temperature} °C | Umid: ${i.humidity}% | Status: ${i.status}`)
  })
  doc.end()
})
...
router.get('/:id/report', requireAuth, async (req, res) => {
  const { id } = req.params
  const m = await prisma.measurement.findUnique({ where: { id }, include: { institution: true, sector: true, user: true } })
...
function avg(list: number[]) { if (!list.length) return 0; return list.reduce((a,b)=>a+b,0)/list.length }
